...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # The gateway runs under pythonw.exe, which owns no console, so a
            # console-subsystem child (node) is handed a brand-new *visible*
            # one — a terminal window pops up for the life of the bridge. Its
            # output already goes to bridge.log, so that console shows nothing
            # and only gets in the user's way. Same helper this file already
            # uses for the netstat/taskkill probes above; 0 on non-Windows.
            bridge_creationflags = 0
            if _IS_WINDOWS:
                from hermes_cli._subprocess_compat import windows_hide_flags

                bridge_creationflags = windows_hide_flags()
            ...
            self._bridge_process = subprocess.Popen(
                ...
                creationflags=bridge_creationflags,
                env=bridge_env,
            )
